<script>
	let {
		tarefas,
		tarefaEditando,
		conteudoTarefaEditando = $bindable(),
		confirmarEdicao,
		cancelarEdicao,
		alterarStatus,
		editarTarefa,
	...
	<div class="input-group mb-1 {tarefa.status == 1 ? 'text-decoration-line-through' : ''}">
		...
		{:else}
			<button
				...
				aria-label="concluir"
				onclick={() => alterarStatus(tarefa, tarefa.status == 0 ? 1 : 0)}
				><i class="bi bi-{tarefa.status == 1 ? 'list-task' : 'check-lg'}"></i></button
			>
			<span class="form-control form-control-lg">{tarefa.conteudo}</span>
			<button
				class="btn btn-warning input-group-text"
				aria-label="editar"
				onclick={() => editarTarefa(tarefa)}><i class="bi bi-pencil"></i></button
			>
			<button
				...
				onclick={() => excluirTarefa(tarefa)}><i class="bi bi-trash"></i></button
			>
		{/if}
	</div>
{/each}
